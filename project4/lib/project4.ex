defmodule Project4 do
    ...
    def loop(data) do
      receive do
        ...
        {:add_block, utx} ->
          ...
          hash = Project4.hashBlock(utx)

          data = if (check(hash,data,length(data) - 1) == false) do
            block = Project4.genBlock(Enum.at(data,length(data)-1),utx)
  ...
  defmodule Server do
    ...
    def setup(gv, numWallets, numTX, iBal) do
      Project4.start_network("serv@")

      #initialize bitcoin stuff (blockchain, etc.)
      block0 = Project4.initBlock()
      ...
      mlist = for _ <- 1..l do
        Task.Supervisor.async(pid,fn() -> mine(sid,Project4.format(gv,nil,blockid)) end)
      end
    ...
    def mine_utxo(serv,utxo,l) do
        hash = Project4.hashTX(utxo)
        if (Project4.verify(hash,4)) do
      ...
      def testx() do
        ...
        IO.puts("check if wallet generated key validates signature")
         ...
         IO.puts("create private key and check if properly gets the public")
          ...
          init = Project4.initBlock()
          block = Project4.genBlock(init,utxo)
          hashval = elem(block,2)
          x = hashval == Project4.hashBlock(utxo)
          IO.puts("#{x}")
      end
  end
